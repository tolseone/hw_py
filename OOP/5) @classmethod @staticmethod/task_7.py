...
class TextInput:
    ...
    def check_name(cls, name):
        return cls.MIN_LEN <= len(name) <= cls.MAX_LEN and all([char in cls.CHARS_CORRECT for char in name]) # аналог set(name) <= set(cls.CHARS_CORRECT)
...
class PasswordInput:
    ...
    @classmethod
    def check_name(cls, name):
        return cls.MIN_LEN <= len(name) <= cls.MAX_LEN and all([char in cls.CHARS_CORRECT for char in name])
...
print(login.login, login.password)

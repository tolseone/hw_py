...
class Stack:
    ...
    @classmethod
    def __valid_data(cls, obj):
        return isinstance(obj, StackObj)

    def push_back(self, obj):
        if not self.__valid_data(obj):
            raise TypeError("должен быть объект класса StackObj")
        if self.top is None:
            self.top = obj
        else:
    ...
    def __iter__(self):  # функция генератор
        h = self.top
        while h:
            yield h
            h = h.next

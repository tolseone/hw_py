...
# Классы StringValue и PriceValue - это дескрипторы данных. Класс StringValue должен проверять, что присваивается строковый тип с длиной строки в 
# диапазоне [2; 50], т.е. min_length = 2, max_length = 50. Класс PriceValue должен проверять,
...
class PriceValue:
    ...
    def validate_value(self, value):
        return type(value) in (int, float) and self.min_value <= value <= self.max_value
...
class SuperShop:
    ...
    def remove_product(self, product):
        if product not in self.goods:
            raise ValueError("Товара с таким названием нет в корзине")
        
        self.goods.pop(self.goods.index(product))


shop = SuperShop("У tolseone")
t = Product("Учебник", 100)
shop.add_product(Product("Курс по Python", 0))
shop.add_product(Product("Курс по Python ООП", 2000))
